...
class LoginWindow:
    def __init__(self, master):
        ...
        self.master.geometry("300x210+500+300")
        self.master.resizable(False, False)

        self.username = StringVar()
        self.password = StringVar()

        Label(master, text="사용자 ID:", font=('맑은 고딕', 11)).pack(pady=5)
        Entry(master, textvariable=self.username,  width=26, font=('맑은 고딕', 11)).pack(pady=2)

        Label(master, text="비밀번호:", font=('맑은 고딕', 11)).pack(pady=5)
        Entry(master, textvariable=self.password, show="*", width=26, font=('맑은 고딕', 11)).pack(pady=2)

        Button(master, text="로그인", font=('맑은 고딕', 11), command=self.check_login).pack(pady=10)
