...
class SimpleStudentSystem:
    ...
    def create_add_student_tab(self):
        ...
        self.notebook.add(frame, text="학생 추가")

        tk.Label(frame, text="새 학생 추가", font=("D2Coding", 16, "bold")).pack(pady=20)

        tk.Label(frame, text="학생 이름:", font=("D2Coding", 12)).pack()
        self.name_entry = tk.Entry(frame, font=("D2Coding", 12), width=30)
        self.name_entry.pack(pady=5)

        tk.Label(frame, text="반:", font=("D2Coding", 12)).pack()
        self.class_entry = tk.Entry(frame, font=("D2Coding", 12), width=30)
        self.class_entry.pack(pady=5)

        tk.Label(frame, text="학번:", font=("D2Coding", 12)).pack()
        self.roll_entry = tk.Entry(frame, font=("D2Coding", 12), width=30)
        self.roll_entry.pack(pady=5)

        tk.Button(frame, text="학생 추가", command=self.add_student,
                  font=("D2Coding", 12, "bold"), bg="green", fg="white", width=28).pack(pady=10)

        tk.Button(frame, text="선택한 학생 정보 업데이트", command=self.update_student,
                  font=("D2Coding", 10), bg="orange", fg="white", width=32).pack(pady=(5, 2))

        tk.Button(frame, text="선택한 학생 삭제", command=self.delete_student,
                  font=("D2Coding", 10), bg="red", fg="white", width=32).pack(pady=(2, 10))

        tk.Label(frame, text="모든 학생:", font=("D2Coding", 14, "bold")).pack(pady=(10, 5))
    ...
    def add_student(self):
        ...
        try:
            ...
            self.db.commit()
            messagebox.showinfo("성공", "학생이 성공적으로 추가되었습니다!")
            self.name_entry.delete(0, tk.END)
            self.class_entry.delete(0, tk.END)
            self.roll_entry.delete(0, tk.END)
            self.refresh_students_list()
        except sqlite3.IntegrityError:
            messagebox.showerror("오류", "학번이 이미 존재합니다! 고유한 학번을 사용해주세요.")
        except Exception as e:
            messagebox.showerror("오류", f"학생 추가 실패: {e}")
    ...
    def update_student(self):
        ...
        if not selected:
            messagebox.showerror("오류", "업데이트할 학생을 목록에서 선택해주세요.")
            return
        ...
        try:
            ...
            messagebox.showerror("오류", "선택한 항목에서 학생 ID를 구문 분석할 수 없습니다.")
            return
        ...
        if not name or not class_name or not roll_no:
            messagebox.showerror("오류", "업데이트를 위해 모든 필드를 채워주세요!")
            return
        ...
        try:
            ...
            if self.cursor.rowcount > 0:
                messagebox.showinfo("성공", "학생이 성공적으로 업데이트되었습니다!")
                self.refresh_students_list()
                self.name_entry.delete(0, tk.END)
                self.class_entry.delete(0, tk.END)
                self.roll_entry.delete(0, tk.END)
            else:
                messagebox.showwarning("경고", "업데이트할 선택된 ID를 가진 학생을 찾을 수 없습니다.")
        except sqlite3.IntegrityError:
            messagebox.showerror("오류", "새 학번이 다른 학생에게 이미 존재합니다!")
        except Exception as e:
            messagebox.showerror("오류", f"학생 업데이트 실패: {e}")
    ...
    def delete_student(self):
        ...
        selected = self.students_listbox.curselection()
        if not selected:
            messagebox.showerror("오류", "삭제할 학생을 목록에서 선택해주세요.")
            return

        confirm = messagebox.askyesno("삭제 확인", "이 학생과 모든 성적을 정말 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.")
        if not confirm:
            return
        ...
        try:
            ...
            messagebox.showerror("오류", "삭제를 위해 선택한 항목에서 학생 ID를 구문 분석할 수 없습니다.")
            return
        ...
        try:
            ...
            self.db.commit()
            if self.cursor.rowcount > 0:
                messagebox.showinfo("성공", "학생과 관련된 성적이 성공적으로 삭제되었습니다!")
                self.refresh_students_list()
                self.name_entry.delete(0, tk.END)
                self.class_entry.delete(0, tk.END)
                self.roll_entry.delete(0, tk.END)
            else:
                messagebox.showwarning("경고", "삭제할 선택된 ID를 가진 학생을 찾을 수 없습니다.")
        except Exception as e:
            messagebox.showerror("오류", f"학생 삭제 실패: {e}")
    ...
    def refresh_students_list(self):
        ...
        try:
            ...
            if not students:
                self.students_listbox.insert(tk.END, "아직 추가된 학생이 없습니다.")
            for student in students:
                student_info = f"ID: {student[0]} | {student[1]} | 반: {student[2]} | 학번: {student[3]}"
                self.students_listbox.insert(tk.END, student_info)
        except Exception as e:
            messagebox.showerror("오류", f"학생 목록 불러오기 실패: {e}")
    ...
    def create_add_marks_tab(self):
        ...
        """
        frame = tk.Frame(self.notebook)
        self.notebook.add(frame, text="성적 추가")

        tk.Label(frame, text="학생 성적 추가 / 업데이트", font=("D2Coding", 16, "bold")).pack(pady=10)
        ...
        tk.Label(frame, text="선택한 학생의 기존 과목:", font=("D2Coding", 12)).pack(pady=(10, 5))
    ...
    def load_student_subjects(self):
        ...
        try:
            ...
            if not subjects:
                self.subjects_listbox.insert(tk.END, "이 학생에게 아직 성적이 추가되지 않았습니다.")
            for subject in subjects:
                self.subjects_listbox.insert(tk.END, subject[0])
        except Exception as e:
            messagebox.showerror("오류", f"학생 과목 불러오기 실패: {e}")
    ...
    def load_selected_subject_data(self, event):
        ...
        try:
            ...
            messagebox.showerror("오류", f"과목 데이터 불러오기 실패: {e}")
    ...
    def add_marks(self):
        ...
        selected_student = self.student_combo.get()
        if not selected_student or "ID: " not in selected_student:
            messagebox.showerror("오류", "먼저 드롭다운에서 학생을 선택해주세요!")
            return
        ...
        if not subject or not marks_text or not skill:
            messagebox.showerror("오류", "성적 입력을 위해 모든 필드를 채워주세요!")
            return

        try:
            marks = int(marks_text)
            if not (0 <= marks <= 100):
                messagebox.showerror("오류", "점수는 0에서 100 사이의 정수여야 합니다.")
                return
            ...
            if self.cursor.fetchone()[0] > 0:
                messagebox.showerror("오류", f"이 학생에게 '{subject}' 과목의 성적이 이미 존재합니다. '업데이트'를 사용하여 변경하세요.")
                return

            query = "INSERT INTO marks (student_id, subject, marks, skill) VALUES (?, ?, ?, ?)"
            self.cursor.execute(query, (student_id, subject, marks, skill))
            self.db.commit()
            messagebox.showinfo("성공", f"{subject} 과목 성적이 성공적으로 추가되었습니다!")
            self.subject_entry.delete(0, tk.END)
            self.marks_entry.delete(0, tk.END)
            self.skill_entry.delete(0, tk.END)
            self.load_student_subjects()
        except ValueError:
            messagebox.showerror("입력 오류", "점수는 유효한 숫자여야 합니다.")
        except Exception as e:
            messagebox.showerror("오류", f"성적 추가 실패: {e}")
    ...
    def update_marks(self):
        ...
        if not new_subject or not new_marks_text or not new_skill:
            messagebox.showerror("오류", "업데이트를 위해 모든 필드(과목, 점수, 기술)를 채워야 합니다.")
            return
        ...
        try:
            ...
            if old_subject_name != new_subject:
                ...
                if self.cursor.fetchone()[0] > 0:
                    messagebox.showerror("오류", f"과목 '{new_subject}'이(가) 이 학생에게 이미 존재합니다. 고유한 과목 이름을 선택하거나 기존 과목을 업데이트하세요.")
                    return
            ...
            self.db.commit()
            if self.cursor.rowcount > 0:
                messagebox.showinfo("성공", f"'{old_subject_name}' 과목 성적이 성공적으로 업데이트되었습니다!")
                self.load_student_subjects()
                self.subject_entry.delete(0, tk.END)
                self.marks_entry.delete(0, tk.END)
                self.skill_entry.delete(0, tk.END)
            else:
                messagebox.showwarning("경고", "업데이트할 선택된 과목에 대한 성적을 찾을 수 없습니다.")
        except ValueError:
            messagebox.showerror("입력 오류", "점수는 유효한 숫자여야 합니다.")
        except Exception as e:
            messagebox.showerror("오류", f"성적 업데이트 실패: {e}")
    ...
    def delete_marks(self):
        ...
        selected_subject_index = self.subjects_listbox.curselection()
        if not selected_subject_index:
            messagebox.showerror("오류", "삭제할 과목을 목록에서 선택해주세요.")
            return

        subject_to_delete = self.subjects_listbox.get(selected_subject_index[0])
        confirm = messagebox.askyesno("삭제 확인", f"'{subject_to_delete}' 과목 성적을 정말 삭제하시겠습니까?")
        if not confirm:
            return
        ...
        try:
            ...
            self.db.commit()
            if self.cursor.rowcount > 0:
                messagebox.showinfo("삭제됨", f"'{subject_to_delete}' 과목 성적이 성공적으로 삭제되었습니다.")
                self.load_student_subjects()
                self.subject_entry.delete(0, tk.END)
                self.marks_entry.delete(0, tk.END)
                self.skill_entry.delete(0, tk.END)
            else:
                messagebox.showwarning("경고", "삭제할 선택된 과목에 대한 성적을 찾을 수 없습니다.")
        except Exception as e:
            messagebox.showerror("오류", f"성적 삭제 실패: {e}")
    ...
    def create_view_results_tab(self):
        ...
        tk.Label(frame, text="학생 결과 보기", font=("D2Coding", 16, "bold")).pack(pady=20)
    ...
    def load_students_combo_and_results_data(self):
        ...
        try:
            ...
            if student_list:
                ...
                self.student_combo.set("사용 가능한 학생이 없습니다")
                self.result_combo.set("사용 가능한 학생이 없습니다")
        except Exception as e:
            messagebox.showerror("오류", f"학생 콤보박스 데이터 불러오기 실패: {e}")
    ...
    def view_result(self):
        ...
        if not selected_student or "ID: " not in selected_student:
            messagebox.showerror("오류", "결과를 보려면 드롭다운에서 학생을 선택해주세요!")
            return
        ...
        try:
            ...
            if not student_data:
                ...
                self.result_text.insert(tk.END, "학생 데이터를 찾을 수 없습니다. (목록에서 선택한 경우 발생해서는 안 됩니다.)")
                return
            ...
            if not marks_data:
                self.result_text.insert(tk.END, f"{student_data[0]} (학번: {student_data[2]}) 학생에게 기록된 성적이 없습니다.")
                return
            ...
            if percentage >= 75:
                academic_performance = "우수 학업 성과"
            elif percentage >= 60:
                academic_performance = "양호 학업 성과"
            elif percentage >= 50:
                academic_performance = "평균 학업 성과"
            else:
                academic_performance = "상당한 개선 필요"
            ...
            messagebox.showerror("오류", f"결과 불러오기 실패: {e}")
    ...
    def create_search_tab(self):
        ...
        tk.Label(frame, text="학생 검색", font=("D2Coding", 16, "bold")).pack(pady=20)

        tk.Label(frame, text="학생 이름 또는 학번 입력:", font=("D2Coding", 12)).pack()
        ...
        tk.Label(frame, text="검색 결과:", font=("D2Coding", 12, "bold")).pack(pady=(20, 10))
    ...
    def search_student(self):
        ...
        if not search_term:
            messagebox.showerror("오류", "검색어를 입력해주세요 (이름 또는 학번)!")
            return
        ...
        try:
            ...
            if not results:
                self.search_listbox.insert(tk.END, "검색어와 일치하는 학생을 찾을 수 없습니다.")
            else:
    ...
    def on_search_result_double_click(self, event):
        ...
        try:
            ...
            if found_item:
                ...
                messagebox.showwarning("찾을 수 없음", "결과 보기 드롭다운에서 학생을 찾을 수 없습니다.")

        except Exception as e:
            messagebox.showerror("오류", f"검색 결과에서 결과 보기 실패: {e}")
    ...
    def create_view_results_tab_index(self):
        ...
        for i, tab_id in enumerate(self.notebook.tabs()):
            if self.notebook.tab(tab_id, "text") == "결과 보기":
                return tab_id
        return None
