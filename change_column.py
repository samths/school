"""
change_column.py 열 순서 변경  Ver 1.0_250731
"""
import pandas as pd
...
class ExcelColumnReorderApp:
    ...
    def create_column_edit_section(self, parent):
        ...
        # 더블클릭 이벤트 바인딩
        self.column_listbox.bind('<Double-1>', self.toggle_column_selection)
    ...
    def create_move_buttons(self, parent):
        ...
        tk.Frame(parent, height=20).pack()  # 간격

        # 선택 관련 버튼
        tk.Label(parent, text="열 선택", font=self.title_font).pack(pady=(0, 5))
    ...
    def browse_input_file(self):
        ...
        if file_path:
            ...
            # 자동으로 출력 파일명 생성
            output_path = file_path.replace(".xlsx", "_reordered.xlsx").replace(".xls", "_reordered.xlsx")
    ...
    def toggle_column_selection(self, event):
        """더블클릭으로 열 선택/해제 토글"""
        index = self.column_listbox.nearest(event.y)
        if index < len(self.current_columns):
...
if __name__ == "__main__":
    ...
    # 윈도우 아이콘 설정 (선택사항)
    try:
        root.iconbitmap('codemy.ico')  # 아이콘 파일이 있는 경우
    except:
        pass
